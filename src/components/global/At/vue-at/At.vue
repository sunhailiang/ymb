...
export default {
  ...
  methods: {
    ...
    // 鼠标进入item
    handleItemHover(e) {
      if (this.isMouseEnter && this.hoverSelect) {
    ...
    handleInput(keep) {
      ...
      const range = getPrecedingRange()
      if (range) {
        const { atItems, allowSpaces, showUnique } = this
    ...
    insertHtml(html, r) {
      ...
      newElement.setAttribute(
        'style',
        'display:inline-block;min-width:4px; margin-left:5px;margin-right:5px; color:#0D84EB'
      )
